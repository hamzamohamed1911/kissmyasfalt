        ...
        <div className="p-4">
          <div className="space-y-6">
            {subTrips.map((subTrip, index) => (
              <div
                key={index}
                className="border border-gray-300 rounded-lg lg:p-4 p-2 bg-white shadow-md flex justify-center items-center flex-row gap-4"
              >
                <Image
                  src={subTrip.subTripImage}
                  alt={subTrip.type}
                  className="w-1/3 lg:h-48 h-40  object-cover rounded-lg"
                />

                <div className="flex-1">
                  <h3 className=" lg:text-3xl md:text-2xl text-xl font-semibold mt-1">{subTrip.type}</h3>
                  <p className="lg:text-lg text-md text-gray-600 md:mt-3 mt-1">
                    {subTrip.description}
                  </p>
                  <p className="lg:text-lg text-md font-semibold md:mt-3 mt-1 text-end">
